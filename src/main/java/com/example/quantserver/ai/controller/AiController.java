...
public class AiController {
    ...
    @Operation(summary = "AI 서버 통신 테스트", description = "더미 데이터로 AI 서버와의 통신을 테스트합니다.")
    @ApiResponses({
    ...
    @GetMapping("/test")
    public ApiResponse<AgentResponse> test(@AuthenticationPrincipal CustomUserDetails userDetails) {
        AgentRequest request = new AgentRequest(userDetails.getId(), "AGGRESSIVE", 10_000_000L);
        return ApiResponse.success(aiServerClient.requestAgent(request));
    }
}